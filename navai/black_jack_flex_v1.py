...
global msg1, msg2, msg3, msg4, name, deck, mi_res, myRules, thicc_cards, deal_dict, player_order, player_bet, total_pot
...
def main():
	...
	if quest1 in yes_list:
		...
		if player_count > 0:
			...
			time.sleep(1)
			credit_dict['DEALER'] = math.inf
			print('SO LETS GET STARTED\n')
			print("DEALER ALSO HAS INFINITE CREDITS")
			time.sleep(3)
			print('CLREAING.........\n')
			time.sleep(2)
			os.system('clear')
...
def blackjack(player_list, credit_dict, score_dict, rounds_count):
	...
	while rounds_count > 0:
		rounds = rounds + 1
		credit_selection = [100, 150, 200, 250, 300, 350, 400, 450, 500]
		mi_cards_list = []
		loser_list = []
		winner_list = []
		winner_dict = {}
		...
		for pl in new_player_list:
			if pl == "DEALER":
				pass
			else:
				bet_quest= int(input('{0}, how much are we agreeing to bet for this round: '.format(pl)))
				player_bet[pl] = bet_quest

		player_bet['DEALER'] = int(random.choice(credit_selection))
		print('DEALER is also betting {0}'.format(player_bet['DEALER']))
		...
		while len(new_player_list) > 0:
			...
			print('{0} - your current credits are {1}.. BEST OF LUCK\n'.format(curr_player, credit_dict[curr_player] - player_bet[curr_player]))
		...
		if len(new_player_list) == 0:
			...
			updateCredits(player_bet, loser_list, winner_list)
			os.system('clear')
			time.sleep(1)
	print(credit_dict, file=fd)
	announceWinner()
...
def updateCredits(player_bet, lsr_list, wnr_list):
	...
	for pl in in_winner_list:
		if score_dict[pl] == 21:
			if len(in_winner_list) > 1:
				temp_crd = player_bet[pl] * 2
				temp_crd_2 = (player_bet[pl] * 2) + credit_dict[pl]
				print('{0} has a hand of 21 but because he/she is not the only player with a hand of 21 - he/she gains {1} which is double his/her bet and has a total of {2}'.format(pl, temp_crd, temp_crd_2))
				credit_dict[pl] = temp_crd_2
				time.sleep(3)
			else:
				temp_crd = player_bet[pl] * 2.5
				temp_crd_2 = (player_bet[pl] * 2.5) + credit_dict[pl]
				print('{0} has a hand of 21 - he/she gains {1} which is double and a half his/her bet and has a total of {2}'.format(pl, temp_crd, temp_crd_2))
				credit_dict[pl] = temp_crd_2
				time.sleep(3)
		else:
			temp_crd = (player_bet[pl] * 2) + credit_dict[pl]
			credit_dict[pl] = temp_crd
			print('{0} has a hand of {1} - he/she gains {2} which is double his/her bet'.format(pl, player_bet[pl], temp_crd))
			time.sleep(3)
'''
	for pl, scr in credit_dict.items():
		if pl in in_loser_list:
			scr1 = scr - player_bet[pl]
			credit_dict[pl] = scr1
			print('{0} has lost {1} credits and has total credits of {2}\n'.format(pl, player_bet[pl], scr1))
			time.sleep(1)
		elif pl in in_winner_list:
			scr1 = scr + in_credit - player_bet[pl]
			credit_dict[pl] = scr1
			print('{0} has gained {1} credits and has total credits of {2}\n'.format(pl, in_credit - player_bet[pl], scr1))
			time.sleep(1)
	time.sleep(5)
'''
