		...
		while True:
			...
			if (round_ctr % 1) == 0:
				...
				while True:
					...
					if mi_card_total <= 21 and mi_card_total  >= 17:
						mi_res = 2
					else:
						if len_in_deck <= 50 and len_in_deck >= 40:
							mi_res = 1
						elif len_in_deck <= 39 and len_in_deck >= 20:
							if mi_card_total  <= 16 and mi_card_total  >= 10:
								if (len(ui_cards) <= 4 and len(mi_cards) <= 4) or (len(ui_cards) <= 4 and re.findall("ace|king|jack|queen", mi_cards)):
									mi_res = 1
								elif ui_cards >= 4 and mi_cards <= 4:
									mi_res = 2
							elif mi_card_total <= 9 and mi_card_total >= 2:
								mi_res = 1
						elif len_in_deck <= 19 and len_in_deck >= 0:
							if mi_card_total <= 16 and mi_card_total >= 5:
								if (len(ui_cards) <= 4 and len(mi_cards <= 4)) or (len(ui_cards) <= 4 and re.findall("ace|king|jack|queen", mi_cards)):
									mi_res = 1
								elif len(ui_cards) >= 4 and len(mi_cards) <= 4:
									mi_res = 2
							elif mi_card_total <= 4 and mi_card_total >= 0:
								mi_res = 1
					if mi_res == 1:
						time.sleep(1)
				...
				if ui_card_total > 21 and mi_card_total > 21:
					...
					time.sleep(2)
					print('WELL THAT WAS ANTI_CLIMACTIC\n')
				elif ui_card_total > 21:
					time.sleep(1)
					...
					time.sleep(2)
					print('NICE ONE{0}\n'.format(name))
				if ui_card_total > mi_card_total:
					time.sleep(1)
