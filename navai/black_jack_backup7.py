...
global msg1, msg2, msg3, msg4, name, deck, mi_res, myRules, thicc_cards, deal_dict, round_bet
...
def blackjack(player_list, credit_dict, score_dict, rounds_count, ):
	...
	while rounds_count > 0:
		...
		turns = 0
		round_bet = int(input('How much are we agreeing for this round: '))
		...
		while len(new_player_list) > 0:
			...
			#print(player_order)
			'''
			if new_player_list == new_player_list2:
				curr_player = random.choice(new_player_list)
				new_player_list.remove(curr_player)
				player_order.append(curr_player)
			elif new_player_list == player_order:
				curr_player = new_player_list[turns]
				turns += turns + 1
				new_player_list.remove(curr_player)
				player_order.append(curr_player)
			'''
				...
				if curr_player != "COMPUTER":
					...
					while True:
						...
						if hit_or_stand == 1:
							...
							score_dict[curr_player] = ui_card_total
							break
					print('score dict = {0}'.format(score_dict))
					os.system('clear')
					time.sleep(2)
					continue
		...
		if len(new_player_list) == 0:
			...
			print(diff_winner_list)
			for pl, val in winner_dict.items():
				if winner_dict[pl] != diff_winner_list[-1]:
					loser_list.append(pl)
				else:
					winner_list.append(pl)
			print('Winner List is ----> {0} and \n Loser List ----> {1}'.format(winner_list,loser_list))
			print('----->UPDATING CREDITS FOR THIS ROUND<-----\n')
			'''
			we have loser_list and winner_list
			what we need to update is, credit_dict, which is a global dict
				if possible, please check credit dict at the begining of the round and remove the player who has
				credits less than 500, assuming 500 is require every round.
			
			'''
			credit = int((round_bet * len(loser_list)) / len(winner_list))
