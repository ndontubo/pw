...
def blackjack(player_list, player_dict, score_dict, rounds_count):
	while rounds_count > 0:
		rounds_count = rounds_count - 1
		new_player_list = player_list.copy()
		new_player_list.append('COMPUTER')
		player_dict['COMPUTER'] = 10000
		init_deal_dict = dealTheCards(new_player_list)
		for n in range(len(new_player_list)):
			...
			print('ALL OTHER PLAYERS LEAVE THIS TABLE, YOU WILL BE CALLED ONCE CURRENT PLAYER IS DONE HIS/HER ROUND')
			print('{0} your current credits are {1}.. BEST OF LUCK'.format(curr_player, player_dict[curr_player]))
			...
			if game_start == '':
				if curr_player != "COMPUTER":
					...
					os.system('clear')
					time.sleep(2)
					print('COMPUTER, these are your cards \n')
					...
					while True:
						if mi_card_total > 21:
							...
							time.sleep(2)
							print(
								'If the user has also busted then it is a tie but if the user has not busted then the user wins\n')
							break
						...
						if mi_res == 1:
							...
							if re.search("(ace_.)", mi_cards[-1]):
								...
								mi_cards[-1] = 10
							mi_card_total = int(mi_card_total) + int(mi_cards[-1])
							time.sleep(2)
							continue
						elif mi_res == 2:
							time.sleep(2)
							...
							time.sleep(2)
							print('COMPUTER, so now it is time to reveal hands\n')
							break
						else:
							time.sleep(2)
							print('COMPUTER, because you are incompetent to select an action you will stand\n')
							time.sleep(2)
							print('COMPUTER, so it is time to reveal hands\n')
							break
					time.sleep(2)
					os.system('clear')
					time.sleep(2)
					...
					time.sleep(2)
					another_match = input(
						'HEY, DO YOU WANNA PLAY ANOTHER GAME {0}, PRESS 3 TO EXIT OR PRESS ANY OTHER KEY TO CONTINUE : '.format(
							name))
					if another_match == '3':
						time.sleep(2)
					...
					while True:
						if mi_card_total > 21:
							...
							time.sleep(2)
							print(
								'But since you went first; you are the dealer and so you only lose if {0} does not bust\n'.format(
									name))
							break
						print(
							"COMPUTER, would you like to 'hit' to gain more cards or 'stand' because you are comfortable with your hand's value\n")
						time.sleep(2)
						print('Press 1 to hit and press 2 to stand : ')
						len_in_deck = len(thicc_cards)
						if 21 >= mi_card_total >= 17:
					...
					while True:
						if ui_card_total > 21:
							...
							time.sleep(2)
							print(
								"But since you have started first in this round and hence you are the dealer you only lose if the computer also busts\n")
							time.sleep(2)
							print("So it is now the computer's turn to select its hand")
							time.sleep(2)
							break
...
def main():
	...
	if quest1 in yes_list:
		...
		if player_count > 0:
			...
			os.system('clear')
			print(msg4)
			blackjack(player_list, player_dict, score_dict, rounds_count)
		else:
			time.sleep(2)
			print('NICE TRY\n')
			time.sleep(1)
